...
def UHFPES(molecules, Na, Nb, basis_set):
    UHF_E = []
    UHF_parameters = []
    spin_flip_parameters = []

    for mol in molecules:
        UE, UHF_par = UHF(mol, Na, Nb, basis_set)
        UHF_E.append(UE)
        UHF_parameters.append(UHF_par.expansion())
    
    for parameters in UHF_parameters:
        flipped_expansion = gqcpy.UTransformation_d(parameters.beta, parameters.alpha)
        spin_flip_parameters.append(flipped_expansion)
    
    return UHF_E, UHF_parameters, spin_flip_parameters
