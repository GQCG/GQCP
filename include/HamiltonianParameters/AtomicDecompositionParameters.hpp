...
struct AtomicDecompositionParameters {
    ...
    static AtomicDecompositionParameters ALT(const HamiltonianParameters<double>& mol_ham_par) {
            ...
            auto t_ab = mol_ham_par.get_ao_basis()->calculateKineticIntegrals() - t_a - t_b;
            auto v_ab = mol_ham_par.get_ao_basis()->calculateNuclearIntegrals() - v_a - v_b;
            ...
            HamiltonianParameters<double> HAA(mol_ham_par.get_ao_basis(), ham_par.get_S(), h_a, g_a, SquareMatrix<double>::Identity(K, K));
            HamiltonianParameters<double> HBB(mol_ham_par.get_ao_basis(), ham_par.get_S(), h_b, g_b, SquareMatrix<double>::Identity(K, K));
            HamiltonianParameters<double> HAB(mol_ham_par.get_ao_basis(), ham_par.get_S(), ham_par.get_h()-h_a-h_b, 2*g_ab, SquareMatrix<double>::Identity(K, K));
            HamiltonianParameters<double> HA(mol_ham_par.get_ao_basis(), ham_par.get_S(), (ham_par.get_h()-h_a-h_b)/2, g_ab, SquareMatrix<double>::Identity(K, K));
