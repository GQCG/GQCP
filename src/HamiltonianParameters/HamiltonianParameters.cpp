 ...
 *  @param C
 */
HamiltonianParameters::HamiltonianParameters(std::shared_ptr<GQCG::AOBasis> ao_basis_ptr, const GQCG::OneElectronOperator& S, const GQCG::OneElectronOperator& h, const GQCG::TwoElectronOperator& g, const Eigen::MatrixXd& C) :
    BaseHamiltonianParameters(std::move(ao_basis_ptr)),
    S (S),
    h (h),
    g (g),
    C (C)
{
    ...
    auto K = this->ao_basis_ptr->number_of_basis_functions;
