 ...
 */
double NuclearFramework::internuclearRepulsionEnergy() const {

    // Sum over every unique nucleus pair
    double value {0.0};
    auto n_nuclei = this->numberOfNuclei();
    for (size_t i = 0; i < n_nuclei; i++) {
        for (size_t j = i + 1; j < n_nuclei; j++ ) {
            ...
            value += nucleus1.charge() * nucleus2.charge() / this->internuclearDistance(i, j);
        }
    }
 ...
 */
Vector<double, 3> NuclearFramework::nuclearDipoleMoment() const {

    Vector<double, 3> m = Vector<double, 3>::Zero();

    for (const auto& nucleus : this->nuclei) {
        m += nucleus.charge() * nucleus.position();
    }
