...
OneRDMs FCIRDMBuilder::calculate1RDMs(const Eigen::VectorXd& x) const {
    ...
    for (auto it_alpha = fock_space_alpha.begin(); it_alpha != end_alpha; ++it_alpha) {
        ...
        for (size_t p = 0; p < K; p++) {  // p loops over SOs
            int sign_p = 1;
            if (onv_alpha.annihilate(p, sign_p)) {  // if p is in I_alpha
                ...
                for (size_t q = 0; q < p; q++) {  // q < p loops over SOs
                    int sign_pq = sign_p;
                    if (onv_alpha.create(q, sign_pq)) {  // if q is not occupied in I_alpha
                        size_t J_alpha = fock_space_alpha.getAddress(onv_alpha);  // find all strings J_alpha that couple to I_alpha
                        ...
                        onv_alpha.annihilate(q);  // undo the previous creation
                    }  // create on q
                }  // q loop
    ...
    for (auto it_beta = fock_space_beta.begin(); it_beta != end_beta; ++it_beta) {
        ...
        for (size_t p = 0; p < K; p++) {  // p loops over SOs
            int sign_p = 1;
            if (onv_beta.annihilate(p, sign_p)) {  // if p is in I_beta
                double diagonal_contribution = 0;
                ...
                for (size_t q = 0; q < p; q++) {  // q < p loops over SOs
                    int sign_pq = sign_p;
                    if (onv_beta.create(q, sign_pq)) {  // if q is not in I_beta
                        ...
                        onv_beta.annihilate(q);  // undo the previous creation
                    }  // create on q
                }  // loop over q
...
TwoRDMs FCIRDMBuilder::calculate2RDMs(const Eigen::VectorXd& x) const {
    ...
    for (auto it_alpha = fock_space_alpha.begin(); it_alpha != end_alpha; ++it_alpha) {
        ...
        for (size_t p = 0; p < K; p++) {  // p loops over SOs
            ...
            if (onv_aaaa.annihilate(p, sign_p)) {  // if p is not in I_alpha

                for (size_t r = 0; r < K; r++) {  // r loops over SOs
                    int sign_pr = sign_p;  // sign of the operator a_r a_p

                    if (onv_aaaa.annihilate(r, sign_pr)) {  // if r is not in I_alpha

                        for (size_t s = 0; s < K; s++) {  // s loops over SOs
                            int sign_prs = sign_pr;  // sign of the operator a^dagger_s a_r a_p

                            if (onv_aaaa.create(s, sign_prs)) {  // if s is in I_alpha

                                for (size_t q = 0; q < K; q++) {  // q loops over SOs
                                    int sign_prsq = sign_prs;  // sign of the operator a^dagger_q a^dagger_s a_r a_p

                                    if (onv_aaaa.create(q, sign_prsq)) {  // if q is not in I_alpha
                                        size_t J_alpha = fock_space_alpha.getAddress(onv_aaaa);  // address of the coupling string
    ...
    for (auto it_alpha = fock_space_alpha.begin(); it_alpha != end_alpha; ++it_alpha) {
        ...
        for (size_t p = 0; p < K; p++) {  // p loops over SOs
            ...
            if (onv_alpha_aabb.annihilate(p, sign_p)) {  // if p is in I_alpha

                for (size_t q = 0; q < K; q++) {  // q loops over SOs
                    int sign_pq = sign_p;  // sign of the operator a^dagger_p_alpha a_p_alpha

                    if (onv_alpha_aabb.create(q, sign_pq)) {  // if q is not in I_alpha
                        size_t J_alpha = fock_space_alpha.getAddress(onv_alpha_aabb);  // the string that couples to I_alpha
                                ...
                                if (onv_beta_aabb.annihilate(r, sign_r)) {
                                    ...
                                    for (size_t s = 0; s < K; s++) {  // s loops over all SOs
                                        ...
                                        if (onv_beta_aabb.create(s, sign_rs)) {
                                            size_t J_beta = fock_space_beta.getAddress(onv_beta_aabb);  // the string that couples to I_beta
    ...
    for (auto it_beta = fock_space_beta.begin(); it_beta != end_beta; ++it_beta) {
        ...
        for (size_t p = 0; p < K; p++) {  // p loops over SOs
            ...
            if (onv_bbbb.annihilate(p, sign_p)) {  // if p is not in I_beta

                for (size_t r = 0; r < K; r++) {  // r loops over SOs
                    int sign_pr = sign_p;  // sign of the operator a_r a_p

                    if (onv_bbbb.annihilate(r, sign_pr)) {  // if r is not in I_beta

                        for (size_t s = 0; s < K; s++) {  // s loops over SOs
                            int sign_prs = sign_pr;  // sign of the operator a^dagger_s a_r a_p

                            if (onv_bbbb.create(s, sign_prs)) {  // if s is in I_beta

                                for (size_t q = 0; q < K; q++) {  // q loops over SOs
                                    int sign_prsq = sign_prs;  // sign of the operator a^dagger_q a^dagger_s a_r a_p

                                    if (onv_bbbb.create(q, sign_prsq)) {  // if q is not in I_beta
