 ...
 */
AP1roGPSEs::AP1roGPSEs(const HamiltonianParameters<double>& ham_par, const size_t N_P) :
    N_P (N_P),
    K (ham_par.get_K()),
    ham_par (ham_par)
{}
 ...
 */
double AP1roGPSEs::evaluateCoordinateFunction(const AP1roGGeminalCoefficients& G, const size_t i, const size_t a) const {
 ...
 */
BlockMatrix<double> AP1roGPSEs::evaluateCoordinateFunctions(const AP1roGGeminalCoefficients& G) const {
    ...
    for (size_t i = 0; i < this->N_P; i++) {
        for (size_t a = N_P; a < this->K; a++) {
            F(i,a) = this->evaluateCoordinateFunction(G, i, a);
        }
    }
 ...
 */
double AP1roGPSEs::evaluateJacobianElement(const AP1roGGeminalCoefficients& G, const size_t i, const size_t a, const size_t j, const size_t b) const {
 ...
 */
BlockRankFourTensor<double> AP1roGPSEs::evaluateJacobian(const AP1roGGeminalCoefficients& G) const {
        ...
        for (size_t a = N_P; a < this->K; a++) {
            for (size_t j = 0; j < this->N_P; j++) {
                for (size_t b = N_P; b < this->K; b++) {
                    J(i,a,j,b) = this->evaluateJacobianElement(G, i, a, j, b);
                }
            }
        }
    }
