...
BOOST_AUTO_TEST_CASE ( H2O_energy_RDM_contraction_FCI ) {
    ...
    double energy_by_contraction = GQCP::calculateExpectationValue(ham_par, one_rdms.one_rdm, two_rdms.two_rdm);  // no scalar energy term in the molecular Hamiltonian parameters, so we're comparing electronic energies
