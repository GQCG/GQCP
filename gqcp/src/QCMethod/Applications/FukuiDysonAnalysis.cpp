        ...
        basis_set (basis_set)
{
    ...
    // Retrieve the LinearExpansions
    const auto LinearExpansion1 = ci_solver1.makeLinearExpansion();
    const auto LinearExpansion2 = ci_solver2.makeLinearExpansion();

    // Extract dyson coefficients
    this->dyson_coefficients = calculateDysonOrbitalCoefficients(LinearExpansion1, LinearExpansion2);
    
    // Calculate the Fukui matrix
    RDMCalculator rdm_calculator1 (fock_space1);
    rdm_calculator1.set_coefficients(LinearExpansion1.get_coefficients());
    RDMCalculator rdm_calculator2 (fock_space2);
    rdm_calculator2.set_coefficients(LinearExpansion2.get_coefficients());
