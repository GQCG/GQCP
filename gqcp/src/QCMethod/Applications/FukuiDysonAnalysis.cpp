        ...
        basis_set (basis_set)
{
    ...
    // In order to supply the correct arguments to the algorithm we choose different ONV basis as fock_space1 should always have the highest occupation to fit the algorithm
    if (open_shell_entry) {
        this->fock_space1 = SpinResolvedONVBasis(K, N_P + 1, N_P);
        this->fock_space2 = SpinResolvedONVBasis(K, N_P, N_P);
    } else {
        this->fock_space1 = SpinResolvedONVBasis(K, N_P, N_P);
        this->fock_space2 = SpinResolvedONVBasis(K, N_P, N_P - 1); 
    }
