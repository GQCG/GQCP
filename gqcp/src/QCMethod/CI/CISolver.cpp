 ...
 */
LinearExpansion CISolver::makeLinearExpansion(size_t index) const {
    if (index >= this->eigenpairs.size()) {
        throw std::logic_error("CISolver::makeLinearExpansion(size_t): Not enough requested eigenpairs for the given index.");
    }
    return LinearExpansion(*this->hamiltonian_builder->get_fock_space(), this->eigenpairs[index].get_eigenvector());
}
