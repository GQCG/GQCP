 ...
 * 
 *  @tparam _ExpansionScalar            the type of scalar used for the expansion coefficients
 */
template <typename _ExpansionScalar>
class DiagonalFockMatrix {
public:
...
public:
     ...
     */
    DiagonalFockMatrix(const SQHamiltonian<ExpansionScalar>& sq_hamiltonian, const double precision=1.0e-08) :
        precision (precision),
     ...
     */
    bool isSatisfiedWith(const QCModel::RHF<ExpansionScalar>& rhf_parameters) const {
        ...
        const auto D = QCModel::RHF<ExpansionScalar>::calculateScalarBasis1RDM(C, 2*N_P);

        // Calculate the Fock matrix in the orthonormal spinor and check if it is diagonal
        auto F_orthonormal = QCModel::RHF<ExpansionScalar>::calculateScalarBasisFockMatrix(D, this->sq_hamiltonian);  // the converged Fock matrix (expressed in the scalar orbital basis)
