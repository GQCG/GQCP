...
private:
    ...
    VectorFunction<double> matrixVectorProduct;
    VectorX<double> diagonal;  // the diagonal of the matrix in question
    MatrixX<double> V_0;  // the set of initial guesses (every column is an initial guess)
...
public:
     ...
     */
    DavidsonSolver(const VectorFunction<double>& matrixVectorProduct, const VectorX<double>& diagonal, const MatrixX<double>& V_0, size_t number_of_requested_eigenpairs = 1, double convergence_threshold = 1.0e-08, double correction_threshold = 1.0e-12, size_t maximum_subspace_dimension = 15, size_t collapsed_subspace_dimension = 2, size_t maximum_number_of_iterations = 128);
     ...
     */
    DavidsonSolver(const VectorFunction<double>& matrixVectorProduct, const VectorX<double>& diagonal, const DavidsonSolverOptions& davidson_solver_options);
    ...
    // GETTERS
    const VectorX<double>& get_diagonal() const { return this->diagonal; };
