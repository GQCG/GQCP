...
public:
    ...
    const ScalarUSQTwoElectronOperator<Scalar>& twoElectronMixed() const { 
        return this->total_two_op.allParameters(GQCP::SpinComponent::ALPHA, GQCP::SpinComponent::BETA); 
    }
    ...
    void transform(const TransformationMatrix<Scalar>& T, const SpinComponent& component) {
        ...
        // Transform the one-electron contributions
        for (auto& one_op : this->one_ops) {
            one_op.allParameters(component).transform(T);
        }

        // transform the two-electron contributions
        for (auto& two_op : this->two_ops) {
            two_op.allParameters(component, component).transform(T);
        }

        // Transform the totals
        this->total_one_op.allParameters(component).transform(T);
        this->total_two_op.allParameters(component, component).transform(T);
    }
    ...
    void rotate(const TransformationMatrix<Scalar>& U, const SpinComponent& component) {
        ...
        // Rotate the one-electron contributions
        for (auto& one_op : this->one_ops) {
            one_op.allParameters(component).rotate(U);
        }

        // Rotate the two-electron contributions
        for (auto& two_op : this->two_ops) {
            two_op.allParameters(component, component).rotate(U);
        }

        // Rotate the totals
        this->total_one_op.allParameters(component).rotate(U);
        this->total_two_op.allParameters(component, component).rotate(U);
    }
