...
/*
 *  MARK: GNonOrthogonalStateBasis
 */

/**
 *  A basis formed by any number of non-orthogonal states, in the form of `GTransformation`s.
 *
...
class GNonOrthogonalStateBasis:
    public CRTP<GNonOrthogonalStateBasis<_Scalar>>,
    public BasisTransformable<GNonOrthogonalStateBasis<_Scalar>>,
    public JacobiRotatable<GNonOrthogonalStateBasis<_Scalar>> {
...
public:
    ...
    using Self = GNonOrthogonalStateBasis<Scalar>;
...
protected:
    ...
    // The overlap operator in AO basis, constructed from the spinor basis.
...
public:
    ...
    /**
     *  Create a `GNonOrthogonalStateBasis` from any number of non orthogonal states.
     *
     *  @param basis_state_vector               The vector containing the non-orthogonal basis states.
     *  @param S_AO                             The overlap operator in AO basis, constructed from the spinor basis used to calculate the non-orthogonal states.
     ...
     */
    GNonOrthogonalStateBasis<Scalar>(const States& basis_state_vector, const OneElectronOperator& S_AO, const size_t number_of_occupied_orbitals, const double threshold = 1e-8) :
