...
public:
    ...
    // The density matrix associated with a generalized basis.
    using DM = G1DM<Scalar>;

    // The representation of this `GLowdinPairingBasis`.
    using Self = GLowdinPairingBasis<Scalar>;
...
private:
    ...
    ScalarGSQOneElectronOperator<Scalar> overlap_operator_AO;
...
public:
     ...
     */
    GLowdinPairingBasis<Scalar>(const Transformation& C_bra, const Transformation& C_ket, const ScalarGSQOneElectronOperator<Scalar>& S_AO, const size_t number_of_electrons, const double threshold = 1e-8) :
