...
public:
    ...
    /**
     *  @param D                the RHF density matrix in a scalar basis basis
     ...
     */
    static double calculateElectronicEnergy(const OneRDM<ExpansionScalar>& D, const ScalarSQOneElectronOperator<ExpansionScalar>& H_core, const ScalarSQOneElectronOperator<ExpansionScalar>& F) {

        // First, calculate the sum of H_core and F (this saves a contraction)
        ScalarSQOneElectronOperator<ExpansionScalar> Z = H_core + F;

        // Convert the matrices Z and D to an Eigen::Tensor<double, 2> D_tensor, as contractions are only implemented for Tensors
        Eigen::TensorMap<Eigen::Tensor<const ExpansionScalar, 2>> D_tensor (D.data(), D.rows(), D.cols());
        ...
        Tensor<ExpansionScalar, 0> contraction = 0.5 * D_tensor.contract(Z_tensor, contraction_pair);
     ...
     */
    static OneRDM<ExpansionScalar> calculateScalarBasis1RDM(const TransformationMatrix<double>& C, const size_t N) {

        const size_t K = C.dimension();
        const auto D_orthonormal = RHF<ExpansionScalar>::calculateOrthonormalBasis1RDM(K, N);
     ...
     */
    static ScalarSQOneElectronOperator<ExpansionScalar> calculateScalarBasisFockMatrix(const OneRDM<ExpansionScalar>& D, const SQHamiltonian<ExpansionScalar>& sq_hamiltonian) {
        ...
        Tensor<ExpansionScalar, 2> direct_contraction = g.contract(D_tensor, direct_contraction_pair);
        Tensor<ExpansionScalar, 2> exchange_contraction = -0.5 * g.contract(D_tensor, exchange_contraction_pair);

        // The previous contractions are Tensor<ExpansionScalar, 2> instances. In order to calculate the total G matrix, we will convert them back into MatrixX<double>
        ...
        return ScalarSQOneElectronOperator<ExpansionScalar>({sq_hamiltonian.core().parameters() + G1 + G2});
    }
     ...
     */
    static ExpansionScalar calculateOrbitalHessianElement(const SQHamiltonian<ExpansionScalar>& sq_hamiltonian, const size_t N_P, const size_t a, const size_t i, const size_t b, const size_t j) {
     ...
     */
    static BlockRankFourTensor<ExpansionScalar> calculateOrbitalHessianTensor(const SQHamiltonian<ExpansionScalar>& sq_hamiltonian, const size_t N_P) {

        const auto K = sq_hamiltonian.dimension();

        BlockRankFourTensor<ExpansionScalar> hessian (N_P,K, 0,N_P, N_P,K, 0,N_P);  // zero-initialize an object suitable for the representation of a virtual-occupied,virtual-occupied object (ai,bj)
            ...
            for (size_t i = 0; i < N_P; i++) {
                ...
                for (size_t b = N_P; b < K; b++) {
                    for (size_t j = 0; j < N_P; j++) {
                        hessian(a,i,b,j) = RHF<ExpansionScalar>::calculateOrbitalHessianElement(sq_hamiltonian, N_P, a, i, b, j);
                    }
                }
            }
        }
    ...
    static size_t LUMOIndex(const size_t K, const size_t N) {
        ...
        return RHF<ExpansionScalar>::HOMOIndex(N) + 1;
    }
