...
BOOST_AUTO_TEST_CASE(density_matrices_traces) {
    ...
    BOOST_CHECK(std::abs(two_DMs.two_rdm_aaaa.trace() - N_alpha * (N_alpha - 1)) < 1.0e-12);
    BOOST_CHECK(std::abs(two_DMs.two_rdm_aabb.trace() - N_alpha * N_beta) < 1.0e-12);
    BOOST_CHECK(std::abs(two_DMs.two_rdm_bbaa.trace() - N_beta * N_alpha) < 1.0e-12);
    BOOST_CHECK(std::abs(two_DMs.two_rdm_bbbb.trace() - N_beta * (N_beta - 1)) < 1.0e-12);


    GQCP::OneDM<double> D_from_reduction = (1.0 / (N - 1)) * two_DMs.two_rdm.reduce();
...
BOOST_AUTO_TEST_CASE(One_DM_from_2_DM) {
    ...
    const auto d = spin_resolved_dm_calculator.calculate2DMs(linear_expansion.coefficients()).two_rdm;
