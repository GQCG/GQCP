...
BOOST_AUTO_TEST_CASE(sandbox){
    ...
    GQCP::VectorX<double> ref_orbital_energies_a {3};  // the STO-3G basisset has 7 basis functions for water
    GQCP::VectorX<double> ref_orbital_energies_b {3};
    ref_orbital_energies_a << -1.03323424,  0.18717761,  0.76900653;
    ref_orbital_energies_b << -0.89036117,  0.82078279,  0.93703007;

    GQCP::TransformationMatrix<double> ref_C_a {3};
    GQCP::TransformationMatrix<double> ref_C_b {3};
    // clang-format off
    ref_C_a << 0.37181816,  1.56954643,  0.89679603,
               0.33197857, -0.00353316, -1.81541885,
               0.37167254, -1.56609373,  0.90287198;
    ref_C_b << -0.30093744,  0.92258904,  1.56978788,
               -0.47100279, -1.78440784, -0.00359124,
               -0.30154051,  0.9290589 , -1.56585144;
    // clang-format on

    // Do our own UHF calculation.
    const auto molecule = GQCP::Molecule::HRingFromDistance(3, 1.0);
    const auto N_alpha = molecule.numberOfElectrons()/2  + 1;
    const auto N_beta = molecule.numberOfElectrons()/2;

    const GQCP::RSpinorBasis<double, GQCP::GTOShell> spinor_basis {molecule, "STO-3G"};
    const auto sq_hamiltonian = GQCP::SQHamiltonian<double>::Molecular(spinor_basis, molecule);  // in an AO basis

    auto uhf_environment = GQCP::UHFSCFEnvironment<double>::WithCoreGuess(N_alpha, N_beta, sq_hamiltonian, spinor_basis.overlap().parameters());
    auto plain_uhf_scf_solver = GQCP::UHFSCFSolver<double>::Plain(1.0e-04, 1000);
    plain_uhf_scf_solver.perform(uhf_environment);


    // Check the calculated results with the reference
    const double total_energy = uhf_environment.electronic_energies.back() + GQCP::Operator::NuclearRepulsion(molecule).value();
    std::cout << total_energy << std::endl;
    std::cout << uhf_environment.orbital_energies_alpha.back() << std::endl;
    std::cout << uhf_environment.orbital_energies_beta.back() << std::endl;
    BOOST_CHECK(std::abs(total_energy - ref_total_energy) < 1.0e-06);

    BOOST_CHECK(GQCP::areEqualEigenvalues(ref_orbital_energies_a, uhf_environment.orbital_energies_alpha.back(), 1.0e-06));
    BOOST_CHECK(GQCP::areEqualEigenvalues(ref_orbital_energies_b, uhf_environment.orbital_energies_beta.back(), 1.0e-06));

    BOOST_CHECK(GQCP::areEqualSetsOfEigenvectors(ref_C_a, uhf_environment.coefficient_matrices_alpha.back(), 1.0e-05));
    BOOST_CHECK(GQCP::areEqualSetsOfEigenvectors(ref_C_b, uhf_environment.coefficient_matrices_beta.back(), 1.0e-05));
