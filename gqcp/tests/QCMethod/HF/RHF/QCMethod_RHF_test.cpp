...
BOOST_AUTO_TEST_CASE(h2_sto3g_szabo_plain) {
    ...
    const GQCP::RSpinOrbitalBasis<double, GQCP::GTOShell> spinor_basis {h2, "STO-3G"};
...
BOOST_AUTO_TEST_CASE(h2o_sto3g_horton_plain) {
    ...
    GQCP::RTransformationMatrix<double> ref_C {7};
    // clang-format off
    ...
    BOOST_CHECK(GQCP::VectorX<double>::areEqualEigenvalues(ref_orbital_energies, rhf_environment.orbital_energies.back(), 1.0e-06));
    BOOST_CHECK(GQCP::MatrixX<double>::areEqualSetsOfEigenvectors(ref_C, rhf_environment.coefficient_matrices.back(), 1.0e-05));
}
...
BOOST_AUTO_TEST_CASE(crawdad_ch4_sto3g_plain) {
    ...
    const GQCP::RSpinOrbitalBasis<double, GQCP::GTOShell> spinor_basis {methane, "STO-3G"};
...
BOOST_AUTO_TEST_CASE(h2_631gdp_plain) {
    ...
    const GQCP::RSpinOrbitalBasis<double, GQCP::GTOShell> spinor_basis {h2, "6-31G**"};
    const auto sq_hamiltonian = GQCP::RSQHamiltonian<double>::Molecular(spinor_basis, h2);  // in an AO basis
