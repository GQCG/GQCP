...
/**
 *  Check if Mulliken-constrained DOCI works. The reference data is taken from @tmhuysen's Master's thesis.
 */
BOOST_AUTO_TEST_CASE ( CO_DOCI_constrained_dense ) {

    Eigen::Matrix<double, 21 , 3> CO_data;  // lambda (multiplier), Mulliken population on C, DOCI energy
    ...
    for (int i = 0; i < 21; i++) {
        ...
        rdm_calc.set_coefficients(doci_dense_eigenvector);
        auto one_rdms = rdm_calc.calculate1RDMs();

        // Retrieve the expectation value of the Mulliken operator (aka the population)
        double mulliken_population = GQCP::calculateExpectationValue(mulliken_operator, one_rdms.one_rdm)[0];

        // Retrieve the total energy by adding the lambda times the expectation value of the constraining operator
        double total_energy = doci_dense_eigenvalue + CO_data(i, 0) * mulliken_population + GQCP::Operator::NuclearRepulsion(CO).value();
