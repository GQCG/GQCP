...
BOOST_AUTO_TEST_CASE ( h2_631gdp ) {
    ...
    // Do an AP1roG calculation in that basis.
    ...
    std::cout << environment.variables.back() << std::endl << std::endl;
    const auto qc_method = GQCP::QCMethod::AP1roG(sq_hamiltonian, N_P);

    std::cout << "Created QC method." << std::endl;

    const auto qc_structure = qc_method.optimize(solver, environment);

    std::cout << "Created QC structure." << std::endl;
