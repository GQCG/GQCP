...
BOOST_AUTO_TEST_CASE(constructor_throws) {
    ...
    const GQCP::UTransformationMatrix<double> T_compatible {GQCP::TransformationMatrix<double>::Zero(K_alpha, K_alpha), GQCP::TransformationMatrix<double>::Zero(K_beta, K_beta)};

    const GQCP::UTransformationMatrix<double> T_incompatible_alpha {GQCP::TransformationMatrix<double>::Zero(K_alpha + 1, K_alpha + 1), GQCP::TransformationMatrix<double>::Zero(K_beta, K_beta)};
    const GQCP::UTransformationMatrix<double> T_incompatible_beta {GQCP::TransformationMatrix<double>::Zero(K_alpha, K_alpha), GQCP::TransformationMatrix<double>::Zero(K_beta + 1, K_beta + 1)};
...
BOOST_AUTO_TEST_CASE(basic_functionality) {
    ...
    BOOST_CHECK(spinor_basis.numberOfSpinors(GQCP::Spin::alpha) == 2);
    BOOST_CHECK(spinor_basis.numberOfSpinors(GQCP::Spin::beta) == 2);
