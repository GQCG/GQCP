...
BOOST_AUTO_TEST_CASE(eval_gto) {
    ...
    for (size_t i = 0; i < nao; i++) {
        BOOST_TEST(AO_vals[i] == AO_vals_ref[i], tt::tolerance(0.000001));
    }
}


// /**
//  *  Check if the LondonGTO evaluation at a random point in space is identical to PySCF results.
//  */
//  BOOST_AUTO_TEST_CASE(eval_gto) {
...
//     // init zero-field because that's the only reference data we have
...
//         AO_vals.push_back(AOs[i](r));
//     }
//     // std::cout << AO_vals. << std::endl;

//     // compare with hardcoded reference data
//     const std::vector<double> AO_vals_ref = {0.0054346, 0.0, 0.000939672, 0.000194685, -0.000584055, 0.002401113, 0.006665074};
//     BOOST_CHECK(AO_vals == AO_vals_ref);
// }