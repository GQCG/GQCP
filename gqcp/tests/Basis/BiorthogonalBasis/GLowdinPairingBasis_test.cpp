...
//     // std::cout << g_spinor_basis.quantize(GQCP::FQMolecularHamiltonian(molecule)).core().parameters() << std::endl;
//     // Initialize two non-orthogonal "Generalised states".
//     GQCP::SquareMatrix<GQCP::complex> C_bra {6};
//     using namespace std::complex_literals;
//     // clang-format off
//     C_bra << 0.460055771  - 5.63405828e-17i,  1.35013939  + 0.0i           ,  0.996501939    - 1.22036291e-16i,  0.535358621 - 6.55625222e-17i,  1.35013939 + 0.0i            ,  0.0 + 0.0i,
...
//     // clang-format off
//     C_ket <<  0.230027886 - 0.398419985i,  0.799802113 + 0.0i        ,  0.498250970    - 0.862995994i   ,  0.267679311 - 0.463634166i,  0.799802113 + 0.0i        , 0.0 + 0.0i,
...
//     // clang-format off
//     std::cout << lowdin_pairing_basis.biorthogonalBraExpansion() << std::endl;
//     // Check the parameters instantiated by the constructor.
//     BOOST_CHECK(lowdin_pairing_basis.biorthogonalBraExpansion().isApprox(biorthogonal_bra_reference, 1e-6));
//     BOOST_CHECK(lowdin_pairing_basis.biorthogonalKetExpansion().isApprox(biorthogonal_ket_reference, 1e-6));
//     BOOST_CHECK(lowdin_pairing_basis.biorthogonalOverlaps().isApprox(overlap_reference, 1e-6));
