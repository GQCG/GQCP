...
// 
#define BOOST_TEST_MODULE "ONVBasis"
...
BOOST_AUTO_TEST_CASE ( ONVBasis_constructor ) {
    ...
    BOOST_CHECK_NO_THROW(GQCP::ONVBasis (10, 5));
}


/**
 *  Test if the expansions generated by the ONV basis are normalized.
 */
BOOST_AUTO_TEST_CASE ( expansions ) {

    // Check the BaseONVBasis expansion functions
    GQCP::ONVBasis fock_space (8, 3);
...
BOOST_AUTO_TEST_CASE ( ONVBasis_dimension ) {
    ...
    BOOST_CHECK_EQUAL(GQCP::ONVBasis::calculateDimension(10, 1), 10);
    BOOST_CHECK_EQUAL(GQCP::ONVBasis::calculateDimension(6, 2), 15);
    BOOST_CHECK_EQUAL(GQCP::ONVBasis::calculateDimension(8, 3), 56);

    BOOST_CHECK_THROW(GQCP::ONVBasis::calculateDimension(100, 50), std::overflow_error);
}


/**
 *  Test if the vertex weights of the ONV basis addressing scheme are correct
 */
BOOST_AUTO_TEST_CASE ( vertex_weights_K5_N3 ) {

    // Let's test an addressing scheme for K=5 and N=3 (5 MOs and 3 alpha electrons)
    GQCP::ONVBasis fock_space = GQCP::ONVBasis(5, 3);
...
/**
 *  Test if the ONV basis correctly counts the amount of coupling ONVs with larger address for a given ONV
 */
BOOST_AUTO_TEST_CASE ( coupling_count ) {
    ...
    GQCP::ONVBasis fock_space2 (16,8);
...
/**
 *  In this test we iterate over the entire ONV basis using the ONVBasis::setNextONV(ONV&) 
 *  and test wether the address is correct using ONVBasis::getAddress(const ONV&) 
 */
BOOST_AUTO_TEST_CASE ( ONV_address_setNext_fullspace ) {

    // Here we will test a full permutation through a ONV basis of K = 15, N = 5
    GQCP::ONVBasis fock_space (15, 5);
...
/**
 *  Test wether the ONV basis attributes the correct address to a given ONV.
 */
BOOST_AUTO_TEST_CASE ( ONVBasis_getAddress ) {

    GQCP::ONVBasis fock_space (6, 3);
...
BOOST_AUTO_TEST_CASE ( ONVBasis_EvaluateOperator_Dense_diagonal_true ) {
    ...
    GQCP::ONVBasis fock_space (6, 4);
    GQCP::ProductONVBasis product_fock_space (6, 4, 0);  // 4 alpha 0 beta product ONV basis as selected ONV basis constructor argument will mimic a spin orbital ONV basis
...
/**
 *  Perform a sparse evaluation of a one-, two-electron operator and the Hamiltonian in the ONV basis (including the diagonal)
 *  and compare these to the selected CI solutions.
 */
...
/**
 *  Perform a sparse evaluation of a one-, two-electron operator and the Hamiltonian in the ONV basis (excluding the diagonal)
 *  and compare these to the selected CI solutions.
 */
